...
echo   1. Backend (FastAPI on :8001)
echo   2. Frontend (Vite on :5173)
echo.

REM Start backend in new window
start "Croppa Backend" cmd /k "cd backend && python -m venv venv && venv\Scripts\activate && pip install -r requirements.txt && uvicorn main:app --reload --port 8001"
